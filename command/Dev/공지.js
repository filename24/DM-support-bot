        ...
        message.channel.send(new MessageEmbed().setTitle(`공지사항`).setDescription(`\`\`\`\n${args.join(" ")}\n\`\`\``).setColor("BLUE")).then(async th => {
            ...
            th.awaitReactions((reaction, user) => (reaction.emoji.name === "❌" || reaction.emoji.name === "⭕") && user.id === message.author.id, {
                ...
                if (collected.array()[0].emoji.name === "⭕") {
                    let result = '';
th.edit(new MessageEmbed().setTitle('📡공지가 전송되었습니다.').setColor('0x00FF46'))
                    client.guilds.cache.forEach(g => {
                        let gc;

                        g.channels.cache.forEach(c => {
                            if (c.name.includes("📜ㅣ공지사항")) gc = c.id;
                        ...
                        try {
                            ...
                            Ch.send(new MessageEmbed().setThumbnail(client.user.displayAvatarURL()).setDescription(args.join(" ")).setColor(0x00FF46).setFooter(message.author.tag, message.author.displayAvatarURL()).setTimestamp())
                        }.catch((e)=>{
                            Hook.send("에러가 발생\n"+e)
